...
# determine the class of examples (introduction to functions, fucntion help)
...
# create a vector of the variety names called var_names and then use
# this vector to name your yield data in yield_irr and yield_drou
# inspect the results
...
# add the two vectors together to detemine furit in each bag,
# and assign to the variable fruit
...
# and assign to the variable total_yield_diff sum()
...
Ireland <- c(15, 15, 17, 19, 20)
France <- c(21, 20, 19, 19, 17)
...
insects <- c("aphids", "butterflies", "hoverflies", "wasps")
...
col_name_vec <- c("col1", "col2", "col3", "col4", "col5")
...
# scale by dividing each element in the matrix by 10
...
# NOTE: a factor stores categorical variables (and not continous variables)
...
mat_one <- matrix(1:25, byrow = TRUE, nrow = 5)
...
my_list <- list(char = vect_char, num = vect_num, my_matrix = mat_one, my_iris = iris_head)


#####  TRY IT OUT  #####
# build your own list (new_list) that contains
...
# from the matrix component of the new_list you made above
